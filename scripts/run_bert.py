#!/usr/bin/env python3
"""
Train AND evaluate BERT (base-uncased) for smishing detection (live training updates).
...
Artifacts (overwrite each run):
  models/bert_base/             (HF model+tokenizer)
...
def main():
    ...
    print("\n[done] BERT trained & evaluated.")
    print("Model  → models/bert_base/")
    print("Logs   → reports/csv/baseline_train_log.csv, reports/csv/baseline_metrics_log.csv")
    print("JSON   → reports/bert_report.json")
